...
func NewGrid() *Grid {
	return &Grid{invalid: true, focusedRow: 1, focusedColumn: 1}
}

func (grid *Grid) SetFocus(row int, column int) {
	grid.focusedRow = row
...
func (grid *Grid) Focus(focus bool) {
...
func (grid *Grid) Event(event tcell.Event) bool {

	for _, cell := range grid.cells {
		if cell.Row == grid.focusedRow && cell.Column == grid.focusedColumn {

			return cell.Content.(Interactive).Event(event)
		}
	}
	return false
}
	...
	case *tcell.EventMouse:
		...
		for _, cell := range grid.cells {
			...
			if x <= localX && localX < x+width && y <= localY && localY < y+height {
				...
				grid.focusedRow = cell.Row
				grid.focusedColumn = cell.Column
