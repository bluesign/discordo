...
func (n *TreeNode) GetAlert() bool {
	return n.alert
}

func (n *TreeNode) SetAlert(alert bool) *TreeNode {
	n.alert = alert
	return n
}
...
func NewTreeView(uiConf *config.UIConfig) *TreeView {
	return &TreeView{
		graphics: true,
		uiConfig: uiConf,
	}
...
func (t *TreeView) process() {
	...
	t.root.Walk(func(node, parent *TreeNode) bool {
		...
		//if len(node.children) == 0 && !node.alert && node!=t.currentNode{
		//	return true
		//}
		if parent == nil {
			node.level = 0
...
func (t *TreeView) Draw(ctx *Context) {
	t.ctx = ctx

	ctx.Fill(0, 0, ctx.Width(), ctx.Height(), ' ', t.uiConfig.GetStyle(config.STYLE_TREE))
	if t.root == nil {
		return
	}
	_, totalHeight := ctx.screen.Size()
	t.process()
	...
	// Draw the tree.
	posY := y
	lineStyle := t.uiConfig.GetStyle(config.STYLE_TREE) //.Background(tcell.ColorWhite).Foreground(tcell.ColorBlack)
	for index, node := range t.nodes {
		...
		if t.graphics {
			...
			for ancestor != nil && ancestor.parent != nil && ancestor.parent.level >= t.topLevel {
				...
				if ancestor.parent.children[len(ancestor.parent.children)-1] != ancestor {
					if posY-1 >= y && ancestor.textX > ancestor.graphicsX {
						PrintJoinedSemigraphics(ctx.screen, ctx.X()+ancestor.graphicsX, ctx.Y()+posY-1, Borders.Vertical, lineStyle)
					}
					if posY < y+height {
						ctx.screen.SetContent(x+ancestor.graphicsX, posY, Borders.Vertical, nil, lineStyle)
					}
				}
			...
			if node.textX > node.graphicsX && node.graphicsX < width {
				...
				if posY-1 >= y && t.nodes[index-1].graphicsX <= node.graphicsX && t.nodes[index-1].textX > node.graphicsX {
					PrintJoinedSemigraphics(ctx.screen, x+node.graphicsX, posY-1, Borders.TopLeft, lineStyle)
				}

				// Join this node.
				if posY < y+height {
					ctx.screen.SetContent(x+node.graphicsX, posY, Borders.BottomLeft, nil, lineStyle)
					for pos := node.graphicsX + 1; pos < node.textX && pos < width; pos++ {
						ctx.screen.SetContent(x+pos, posY, Borders.Horizontal, nil, lineStyle)
					}
				}
			}
		}
		...
		if node.textX < width && posY < y+height {
			...
			var prefixWidth int
			if len(t.prefixes) > 0 {
				_, prefixWidth = Print(ctx.screen, t.prefixes[(node.level-t.topLevel)%len(t.prefixes)], ctx.X()+x+node.textX, ctx.Y()+posY, width-node.textX, AlignLeft, node.color)
			}

			// Text.
			if node.textX+prefixWidth < width {
				style := t.uiConfig.GetStyle(config.STYLE_TREE)

				if node.alert {
					style = t.uiConfig.GetComposedStyle(config.STYLE_TREE, []config.StyleObject{config.STYLE_TREE_ALERT}) // t.uiConfig.GetStyle(config.STYLE_TREE_ALERT)
				...
				printWithStyle(ctx.screen, node.text, ctx.X()+x+node.textX+prefixWidth, ctx.Y()+posY, 0, width-node.textX-prefixWidth, AlignLeft, style, false)
			}
		}
...
func (ti *TreeView) Invalidate() {
	ti.DoInvalidate(ti)
}

func (acct *TreeView) Focus(focus bool) {
}

func (t *TreeView) Event(event tcell.Event) bool {
...
func (t *TreeView) MouseEvent(localX int, localY int, event tcell.Event) {
