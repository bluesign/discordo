...
func NewTextView(uiConf *config.UIConfig) *TextView {
	return &TextView{
...
func (t *TextView) Focus(focus bool) {
	t.focused = focus
}
...
func (t *TextView) Draw(ctx *Context) {
	...
	ctx.Fill(0, 0, ctx.Width(), ctx.Height(), ' ', t.uiConfig.GetStyle(config.STYLE_TEXTVIEW))
	...
	totalWidth, totalHeight := ctx.screen.Size()
	...
	// Draw the buffer.
	defaultStyle := t.uiConfig.GetStyle(config.STYLE_TEXTVIEW) //tcell.StyleDefault.Foreground(t.textColor).Background(tcell.ColorBlack)
		...
		if y+line-t.lineOffset >= 0 {
			...
			iterateString(strippedText, func(main rune, comb []rune, textPos, textWidth, screenPos, screenWidth int) bool {
				...
				for offset := screenWidth - 1; offset >= 0; offset-- {
					if offset == 0 {
						ctx.screen.SetContent(x+posX+offset, y+line-t.lineOffset, main, comb, style)
					} else {
						ctx.screen.SetContent(x+posX+offset, y+line-t.lineOffset, ' ', nil, style)
					}
				}
...
func (t *TextView) Event(event tcell.Event) bool {
	if !t.focused {
		return false
	}
	...
	case *tcell.EventKey:
		...
		case tcell.KeyRune:
			...
			case 'G': // End.
				...
				t.columnOffset = 0
			case 'j':
				if len(t.GetHighlights()) > 0 {
					highlight := t.GetHighlights()[0]
					for k, v := range t.regionInfos {
						if highlight == v.ID {
							k = k + 1
							if k >= len(t.regionInfos) {
								k = len(t.regionInfos) - 1
							}

							t.Highlight(t.regionInfos[k].ID).ScrollToHighlight()
							t.Invalidate()
						}
					}
				}
...
func (t *TextView) MouseEvent(localX int, localY int, event tcell.Event) {
	switch event := event.(type) {
